...
class StackReviewTests: XCTestCase {
  ...
  func testSetFavorite() {
    ...
    XCTAssert(collection.isFavorite(pancakeHouse))
  ...
  func testSetFavoriteNotInCollection() {
    ...
    let pancakeHouse = PancakeHouse(dict: pancakeData)!
  ...
  func testCloudLoadSucceeds() {
    ...
    let expectation = expectationWithDescription("Expecting cloud data call to succeed!")
